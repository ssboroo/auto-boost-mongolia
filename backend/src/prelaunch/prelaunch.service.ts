...
export class PrelaunchService {
  ...
  async run(req: any) {
    ...
    const checks: Check[] = []
    const frontendRaw = process.env.FRONTEND_ORIGIN || ''
    const redirectRaw = process.env.META_REDIRECT_URI || ''
    const frontend = this.cleanUrl(frontendRaw)
    const redirect = this.cleanUrl(redirectRaw)
    const supabaseUrl = this.cleanUrl(process.env.SUPABASE_URL || '')
    ...
    const frontendValid = frontend.startsWith('https://')
    const expectedRedirect = frontendValid ? `${frontend}/api/meta/auth/callback` : 'https://auto-boost-mongolia.vercel.app/api/meta/auth/callback'
    ...
    if (supabaseUrl && process.env.SUPABASE_PUBLISHABLE_KEY) {
      try {
        ...
        checks.push(this.check('supabase_reachable', 'Supabase API', reachable ? 'READY' : 'ERROR', reachable ? `Supabase reachable · HTTP ${response.status}` : `Supabase HTTP ${response.status}`))
      } catch (e: any) {
    ...
    try {
      const response = await fetch('https://www.mongolbank.mn/en/', { headers: { 'User-Agent': 'AutoBoostMongolia-Prelaunch/1.0' }, signal: AbortSignal.timeout(10000) })
